            ...
            {cart.map((item) => (
              <div key={item.id}>
                <div className="flex justify-between items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.toppings && item.toppings.length > 0 && (
                      <p className="text-[11px] text-gray-500 mt-0.5">
                        + {item.toppings.join(' · ')}
                        <span className="text-orange-400"> (+{(item.toppings.length * TOPPING_PRICE).toLocaleString()}đ)</span>
                      </p>
                    )}
                    {item.combo && (
                      <p className="text-[11px] text-emerald-600 mt-0.5">🍹 {item.combo}</p>
                    )}
                    <p className="text-orange-600 text-xs font-bold mt-0.5">{item.price.toLocaleString()}đ/phần</p>
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 bg-gray-50 p-1 rounded-xl">
                      ...
                      <span className="text-black w-4 text-center text-sm font-bold">{item.quantity}</span>
                ...
                {expandedNoteId !== item.id && item.note && (
                  <p className="text-[11px] text-orange-500 mt-1">📝 {item.note}</p>
                )}
              </div>
            ))}
          </div>
